...
from system.utils.platform import get_system
from system.utils.filesystem import ensure_directory, copy_file, file_exists
from system.utils.subprocess_helper import check_extension_installed, install_extension, run_python_script
from system.utils.config import get_config, get_required_directories, get_extensions
...
def run_vibe_check() -> None:
    ...
    vibe_script = os.path.join("system", "scripts", "vibe_check.py")
...
def run_skill_optimizer() -> None:
    ...
    script = os.path.join("system", "scripts", "optimize_skills.py")
...
def run_structure_enforcement() -> None:
    ...
    script = os.path.join("system", "scripts", "enforce_structure.py")
    if file_exists(script):
        run_python_script(script)
