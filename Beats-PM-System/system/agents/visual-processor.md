...
Analyze visual inputs (screenshots, images, diagrams) captured in `0. Incoming/` to extract content and context, then route to the correct agent.
...
**Trigger**: `#screenshot`, `#clipboard`, or manual drop in `0. Incoming/`.

1.  **Scan**: Check `0. Incoming/` for new images.
...
1.  **Source**: "Screenshot"
2.  **Product Context**: (Scan `2. Products/` for keywords in the image)
