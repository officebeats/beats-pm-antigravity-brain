...
export const StatusPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on("status-update", (data: { content: string }) => {
      setContent(data.content);
      setLastUpdated(new Date());
      console.log("🔥 Brain pushed Action Plan update.");
    });
          ...
          <div className="prose prose-clay max-w-none">
            <ReactMarkdown
              components={{
                h1: ({ node, ...props }) => (
                  <h1
                    className="text-3xl font-black text-clay-foreground mb-6"
                    {...props}
                  />
                ),
                h2: ({ node, ...props }) => (
                  <h2
                    className="text-xl font-black text-clay-foreground mt-8 mb-4 flex items-center gap-2"
                    {...props}
                  />
                ),
                p: ({ node, ...props }) => (
                  <p
                    className="text-clay-muted font-medium leading-relaxed mb-4"
                    {...props}
                  />
                ),
                ul: ({ node, ...props }) => (
        ...
        {!content && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <p className="text-clay-muted font-medium max-w-xs">
              The system is currently listening for your next move. Send a
