    ...
    <div className="flex flex-col h-full w-full overflow-hidden bg-white/40 backdrop-blur-md rounded-[32px] border border-white/50 shadow-clay-card">
      ...
      >
        {history.length === 0 && !currentThinking && (
          <div className="h-full flex flex-col items-center justify-center text-clay-muted text-center p-8">
            <div className="relative mb-8 group">
              <div className="absolute inset-0 bg-clay-accent/20 blur-3xl rounded-full scale-150 group-hover:bg-clay-accent/30 transition-all duration-1000" />
              <Brain className="w-20 h-20 text-clay-accent relative z-10 animate-clay-breathe" />
              <div
                className={clsx(
                  "absolute -bottom-2 -right-2 w-6 h-6 rounded-full border-4 border-white shadow-lg",
                  isConnected
                    ? "bg-clay-accent-success"
                    : "bg-clay-accent-warning"
                )}
              />
            </div>
            <h3 className="font-display font-black text-3xl text-clay-foreground tracking-tight mb-2">
              Neural Mesh Online
            </h3>
            <p className="font-medium text-clay-muted max-w-xs leading-relaxed opacity-60">
              The Beats Antigravity Brain is awaiting your strategic directives.
            </p>
          </div>
        )}
        ...
        {currentThinking && (
          <div className="flex flex-col items-start animate-pulse">
            <div className="flex items-center gap-3 mb-2 px-4">
              <Rocket className="w-4 h-4 text-clay-accent-secondary animate-bounce" />
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-clay-accent-secondary">
      ...
      >
            ...
            {images.map((img, idx) => (
              <div key={idx} className="relative flex-none group">
                ...
                <button
                  ...
                  className="absolute -top-2 -right-2 bg-clay-accent-secondary text-white rounded-full p-1 shadow-lg"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
        ...
        <div className="flex items-end gap-4">
          <textarea
            ...
            className="flex-1 bg-transparent border-0 focus:ring-0 text-clay-foreground text-lg placeholder:text-clay-muted/20 resize-none max-h-[120px] font-semibold py-2"
          />
