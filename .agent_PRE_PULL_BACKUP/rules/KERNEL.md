...
**Version**: 4.11.0 (Stability & Performance)
...
1. **O(1) Retrieval**: Before using `find_by_name` or `list_dir` to locate a document (PRD, Strategy, Previous Meeting), the Agent **MUST** first read `system/content_index.json`.
...
3. **Conductor-First Protocol (CRITICAL)**:
   - **Rule**: Whenever a new document, spec, report, or tracker entry is created, the system MUST check `.agent/templates/` first.
...
1. **Expert Activation**: When a specific intent (Bug, PRD, Meeting, Task, Refactor) is detected, the system MUST activate the corresponding skill from `.agent/skills/` using `activate_skill`.
...
12. **System Updates**: If input is `#update`, execute `git pull && npm install -g @google/gemini-cli@preview` followed by `python system/scripts/core_setup.py`.
