...
[DROP_ZONE, INBOX_DIR, REQUESTS_DIR, RESPONSES_DIR, SCREENSHOTS_DIR].forEach(dir => {
...
fs.watch(RESPONSES_DIR, (event, filename) => {
    if (event === 'rename' && filename && filename.endsWith('.md')) {
        ...
        if (fs.existsSync(filePath)) {
            ...
            io.emit('response', { filename, content, timestamp: new Date().toISOString() });
        }
    }
});
...
app.post('/api/chat', chatUpload.array('images'), (req, res) => {
    try {
        ...
        io.emit('user-message', { content: message, timestamp: new Date().toISOString() });

        // --- SIMULATE BRAIN THINKING & RESPONDING ---
        // This simulates an external agent picking up the file
        setTimeout(() => {
            const timestamp = new Date().toLocaleTimeString();
            fs.appendFileSync(THINKING_LOG, `\n[${timestamp}] 🧠 Brain acknowledged: "${message.substring(0, 30)}..."\n`);
            
            setTimeout(() => {
                fs.appendFileSync(THINKING_LOG, `🔍 Scanning product portfolio and stakeholders...\n`);
                
                setTimeout(() => {
                    fs.appendFileSync(THINKING_LOG, `✨ Generating strategic response...\n`);
                    ...
                    setTimeout(() => {
                        ...
                        fs.writeFileSync(path.join(RESPONSES_DIR, responseFilename), responseContent);
...
app.get('/api/history', (req, res) => {
    try {
        const requests = fs.readdirSync(REQUESTS_DIR)
        ...
        const history = [...requests, ...responses].sort((a, b) => 
