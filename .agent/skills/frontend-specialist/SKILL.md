...
description: Master of UI/UX, React, TypeScript, and modern CSS.
triggers:
  - "/ux"
  - "/frontend-build"
version: 1.0.0
...
- **Inputs**: File Paths, Design guidelines, `/ux` (Build UI).
- **Outputs**: High-quality `.tsx` / `.jsx` / `.css` components.
...
2. Ensure semantic HTML structure (aside from just `div` soup).
...
When the `/ux` workflow is triggered, implement the UI requested by the user adhering strictly to these frontend best practices, optimizing the interface for the end user immediately.
