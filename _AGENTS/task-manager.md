...
When processing `BRAIN_DUMP.md` or random user input:

| Input Type              | Action                                               |
| ----------------------- | ---------------------------------------------------- |
| **Clear & Actionable**  | Extract task → Add to `ACTION_PLAN.md` with priority |
| **Needs Clarification** | Add to `_QUEUE/needs-clarification.md` with question |
| **Pure Idea**           | Keep in `BRAIN_DUMP.md`, tag as `💭 Idea`            |
...
Maintain `_QUEUE/needs-clarification.md`:
...
1. **Aggregate** from all sources (`BUGS/`, `CRITICAL/`, `PROJECTS/`, `PEOPLE/`, `_QUEUE/`)
...
| **All Agents**              | Can add to `_QUEUE/needs-clarification.md`      |
...
| `_QUEUE/needs-clarification.md` | Items awaiting user input                         |
| `_QUEUE/pending-triage.md`      | Raw items from brain dump awaiting processing     |
...
1. **On `#day`**:
   ...
   - Run triage on any new `BRAIN_DUMP.md` content
...
2. ❓ **Needs Clarification**: "Blocked on API - who owns it?" → `_QUEUE/needs-clarification.md`
